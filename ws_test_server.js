...
io.on('connection', function (socket) {
        ...
        socket.emit('blabla', { type: 'user:login1', data: socket.user });

    setTimeout(() => {
    //   this.socket.on('blabla', msg => {
        socket.emit('blabla', { type: 'user:login2', data: socket.user });
    //   });
    }, 5000)
    
    socket.on('app:ready', function() {
        socket.emit('app:state', { data: 'user:login3' });
        socket.emit('app:songlist', { data: 'user:login4' });
    })
